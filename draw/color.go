...
func ARGB(alpha float64, red, green, blue int) Color {
	return Color(clamp0To255(blue)<<24 | clamp0To255(green)<<16 | clamp0To255(red)<<8 | clamp0To1AndScale255(alpha))
}

// ARGBfloat creates a new Color from ARGB values in the range 0-1.
func ARGBfloat(alpha, red, green, blue float64) Color {
	return Color(clamp0To1AndScale255(blue)<<24 | clamp0To1AndScale255(green)<<16 | clamp0To1AndScale255(red)<<8 | clamp0To1AndScale255(alpha))
}
...
// ColorDecode creates a Color from a string. The string may be in any of the
// standard CSS formats:
//
...
func ColorDecode(buffer string) Color {
	...
	case strings.HasPrefix(buffer, "rgba(") && strings.HasSuffix(buffer, ")"):
		...
		if len(parts) == 4 {
			return ARGB(extractAlpha(parts[3]), extractChannel(parts[0], 10), extractChannel(parts[1], 10), extractChannel(parts[2], 10))
		}
...
// String implements the fmt.Stringer interface. The output can be used as a
// color in CSS.
...
// RGBA implements color.Color. Assumes the color not premultiplied and must
// be premultiplied to meet the contract of color.Color.
...
// SetAlpha returns a new color based on this color, but with the alpha
// channel replaced.
func (c Color) SetAlpha(alpha int) Color {
	return Color(clamp0To255(alpha) | (int(c) & 0xFFFFFF00))
}
...
// SetAlphaIntensity returns a new color based on this color, but with the
// alpha channel replaced.
func (c Color) SetAlphaIntensity(alpha float64) Color {
	return ARGB(alpha, c.Red(), c.Green(), c.Blue())
}

// Monochrome returns true if each color component is the same, making it a
// shade of gray.
...
// Lower values represent darker colors, while higher values represent
// brighter colors.
...
func (c Color) Premultiply() Color {
	...
	default:
		...
		b *= a
		b /= 0xff
		return Color((b & 0xff) | (g & 0xff) | (r & 0xff) | a)
	}
}

// Unpremultiply divides the color channels by the alpha channel, effectively
// undoing a Premultiply call. Note that you will not necessarily get the
// original value back after calling Premultiply followed by Unpremultiply.
